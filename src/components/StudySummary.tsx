...
import { reminderCounts } from '../lib/reminders'
import { localDate } from '../types/study'
...
export function StudySummary() {
  ...
  useEffect(()=>{const tick=()=>setToday(localDate());const id=setInterval(tick,30000);window.addEventListener('focus',tick);return()=>{clearInterval(id);window.removeEventListener('focus',tick)}},[])
  const reminders=reminderCounts(tasks,today)

...
const mocks=vi.hoisted(()=>({single:vi.fn(),insert:vi.fn(),upsert:vi.fn()}))
...
vi.mock('../lib/loadOwned',()=>({loadOwned:async()=>[]}))
vi.mock('../lib/supabase',()=>({client:()=>({from:()=>{const q={select:()=>q,eq:()=>q,abortSignal:()=>q,maybeSingle:async()=>({data:null,error:null}),insert:(v:unknown)=>{mocks.insert(v);return q},upsert:(v:unknown)=>{mocks.upsert(v);return q},single:mocks.single};return q}})}))
import Focus from './Focus'
beforeEach(()=>{vi.clearAllMocks();sessionStorage.clear();mocks.single.mockResolvedValue({data:{id:'ok'},error:null})})
afterEach(cleanup)

...
export default function Register() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault(); if (blocked || busy) return; const form = e.currentTarget; const data = new FormData(form)
    ...
    setBusy(true)
    try {
      const { data: result, error } = await client().auth.signUp({ email, password, options: { data: { full_name: name }, emailRedirectTo: `${window.location.origin}/login` } })

...
export default function Study({ mode }: { mode: 'courses' | 'tasks' }) {
 ...
 const pagination = usePagination(shown, JSON.stringify([query,filter]))
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (!isCourse && Number(form.get('repeat_days')) > 0 && !form.get('due_date')) { setError('Informe um prazo para repetir a tarefa.'); return }
    setBusy(true); setError(''); setSuccess('')
    try {
      const fields = isCourse ? { title, institution: String(form.get('institution')).trim(), hours: Number(form.get('hours')), progress: Number(form.get('progress')), url: safeCourseUrl(String(form.get('url'))) } : { title, course_id: String(form.get('course_id')) || null, due_date: String(form.get('due_date')) || null, priority: String(form.get('priority')), status: String(form.get('status')), repeat_days: Number(form.get('repeat_days')) }
